module RoomsHelper
  ...
  def most_new_message_preview(room)  
    ...
    if message.present?
      # メッセージがあれば内容を表示
      tag.p "#{message.content.truncate(30)}", class: "dm_list__content__link__box__message"
    else
  ...
  def time(room)
    ...
    if message.present?
      # メッセージがあれば内容を表示
      tag.p "#{time_ago_in_words(message.updated_at)}前", class: "dm_list__content__link__box__message_time"
    else
      # メッセージがなければ[ まだメッセージはありません ]を表示
      tag.p "[ まだメッセージはありません ]", class: "dm_list__content__link__box__message_time"
    end
  end
